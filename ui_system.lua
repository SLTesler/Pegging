...
local function drawPopperIcon(popperId, x, y, time, scale)
    ...
    elseif popperId == "bananaPopper" then
        ...
        love.graphics.pop()
    elseif popperId == "airtimePopper" then
        -- Draw a stylized stopwatch/clock with a swirl for airtime
        love.graphics.setColor(0.6, 0.9, 1) -- Light blue outer ring
        love.graphics.circle("fill", x + 40*scale, y + 44*scale, 28*scale)
        love.graphics.setColor(1, 1, 1) -- White face
        love.graphics.circle("fill", x + 40*scale, y + 44*scale, 20*scale)
        -- Clock hands
        love.graphics.setColor(0.2, 0.4, 0.8)
        local handAngle = (time * 4) % (2*math.pi)
        love.graphics.setLineWidth(4*scale)
        love.graphics.line(x + 40*scale, y + 44*scale, x + 40*scale + math.cos(handAngle) * 16*scale, y + 44*scale + math.sin(handAngle) * 16*scale)
        love.graphics.setLineWidth(2*scale)
        love.graphics.line(x + 40*scale, y + 44*scale, x + 40*scale + math.cos(handAngle*2) * 10*scale, y + 44*scale + math.sin(handAngle*2) * 10*scale)
        love.graphics.setLineWidth(1)
        -- Swirl effect for "air"
        for i = 1, 8 do
            local swirlAngle = handAngle + i * math.pi/4 + time
            local swirlRadius = 24*scale + 4*math.sin(time*2 + i)
            love.graphics.setColor(0.6, 0.9, 1, 0.25)
            love.graphics.circle("fill", x + 40*scale + math.cos(swirlAngle) * swirlRadius, y + 44*scale + math.sin(swirlAngle) * swirlRadius, 4*scale)
        end
    elseif popperId == "wallMasterPopper" then
        -- Draw a stylized blue wall with a big white '5x' on it
        love.graphics.setColor(0.3, 0.7, 1)
        love.graphics.rectangle("fill", x + 20*scale, y + 32*scale, 40*scale, 28*scale, 8*scale, 8*scale)
        love.graphics.setColor(0.2, 0.4, 0.7)
        love.graphics.rectangle("fill", x + 20*scale, y + 54*scale, 40*scale, 8*scale, 4*scale, 4*scale)
        love.graphics.setColor(1, 1, 1)
        love.graphics.setFont(love.graphics.newFont(28*scale))
        love.graphics.printf("5x", x + 20*scale, y + 36*scale, 40*scale, "center")
        love.graphics.setFont(love.graphics.getFont()) -- Reset font
    end
end
