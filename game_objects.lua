...
function gameObjects.createExplosion(gameState, x, y)
    for i = 1, 10 do
        ...
        -- Update round state for explosion balls
        gameState.currentRound.active_balls = gameState.currentRound.active_balls + 1
    end
end
...
function gameObjects.updateBalls(gameState, dt)
    ...
    for i = #gameState.balls, 1, -1 do
        ...
        -- Airtime Popper: award points every 0.2s in air
        if gameState.poppers and (gameState.poppers.airtimePopper or 0) > 0 then
            ball.airtimeTimer = (ball.airtimeTimer or 0) + dt
            while ball.airtimeTimer >= 0.2 do
                ball.airtimeTimer = ball.airtimeTimer - 0.2
                gameState.currentRound.score = gameState.currentRound.score + config.SCORING.AIRTIME_POPPER_POINTS
                if visualEffects and visualEffects.addEffect then
                    visualEffects.addEffect(gameState, ball.x, ball.y - 30, "+" .. config.SCORING.AIRTIME_POPPER_POINTS .. " AIRTIME!", {0.6, 0.9, 1})
                end
            end
        end
    end
    -- Assert that active_balls matches the number of balls in play
    assert(gameState.currentRound.active_balls == #gameState.balls, "active_balls desync: " .. tostring(gameState.currentRound.active_balls) .. " vs " .. tostring(#gameState.balls))
end
