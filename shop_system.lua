...
shopSystem.POPPERS = {
    ...
    {id = "airtimePopper", name = "Airtime Popper", desc = "Gain +15 points every 0.2 seconds the ball is in the air.", price = 28, rarity = 0.7}
}
...
function shopSystem.purchaseItem(gameState, item)
    ...
    if gameState.coins >= item.price then
        ...
        elseif item.id == "multiplierBoost" then
            ...
            gameState.coins = gameState.coins - item.price
        elseif item.id == "airtimePopper" then
            gameState.poppers.airtimePopper = 1
            gameState.coins = gameState.coins - item.price
        else
...
function shopSystem.purchaseCandy(gameState, candy)
    if candy.id == "candyLife" then
        ...
        if gameState.coins >= candy.price then
            gameState.permanentExtraLives = (gameState.permanentExtraLives or 0) + 1
            gameState.lives = gameState.lives + 1
